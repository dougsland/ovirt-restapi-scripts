...
if __name__ == "__main__":
	...
	if len(sys.argv) < 7:
		print "Usage: %s StorageNAME MyHost LogicalUnitId MyStorage_Address MyStorage_Port Target UserName (optional) Password (optional)" %(sys.argv[0])
		print "Example: %s MyNEWISCSIStorage 192.168.1.60 35000144f35295034 192.168.1.103 3260 iqn.1992-04.com.emc:storage.storage.iscsi15g" %(sys.argv[0])
		sys.exit(1)
	...
	if argc > 7:
		...
		<storage_domain>
			...
			<storage>
				<type>iscsi</type>
				<logical_unit id="""+"\""+ """"""+sys.argv[3]+""""""+ "\""+""">
					<address>""" + sys.argv[4] + """</address>
					<port>""" + sys.argv[5] + """</port>
					<target>""" + sys.argv[6] + """</target>
					<username>""" + sys.argv[7] + """</username>
					<password>""" + sys.argv[8] + """</password>
				</logical_unit>
			</storage>
		</storage_domain>
		"""
	...
	try:
		...
		print "- Are you trying to add < 10G iscsi storage? (It's won't supported)"
