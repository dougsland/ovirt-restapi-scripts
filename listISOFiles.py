...
if __name__ == "__main__":
	...
	if len(sys.argv) != 2:
		print "Usage: %s my_storage_iso" %(sys.argv[0])
		print "Example: %s myDomainISO" %(sys.argv[0])
		sys.exit(1)

	id_st = getStorageId(sys.argv[1])
	...
	# Setting URL
	URL      = "https://" + ADDR + ":" + API_PORT + "/api/storagedomains/" + id_st + "/files"
	...
	try:
		...
		print "%s" %(e)
		print "Are you trying to use a non ISO storage?"
		sys.exit(-1)
...
list = tree.findall("file")

st_id = None
for item in list:
	print "ISO: %s" % (item.find("name").text)
