...
def getStorageId(st_name):
	...
	print "Preparing to remove: %s" %(st_name)
	...
	for item in list:
		if st_name == item.find("name").text:
			...
			print "storage id %s" % (st_id)
			break

	return st_id


def removeStorage(st_id):

	# Setting URL
	URL      = "https://" + ADDR + ":" + API_PORT + "/api/storagedomains/" + str(st_id)
...
if __name__ == "__main__":
	...
	if len(sys.argv) != 2:
		print "Usage: %s my_unattached_storage" %(sys.argv[0])
		sys.exit(1)

	id_ret = getStorageId(sys.argv[1])

	if id_ret == None:
		print "Cannot find Storage"
		sys.exit(1)

	ret = removeStorage(id_ret)
	if ret == 0:
